...
lgd_in_file = external_path.joinpath("LocalBodies.csv")
...
def lgd_master():
    def lgd_file_prep():
        ...
        lgd["state_block_localbody"] = (
            lgd["state"] + lgd["block"] + lgd["localbody"]
        )
    ...
    def gp_name_clean():
        ...
        conditions = [
            ...
            & (df["district"] == "LOWER SUBANSIRI")
            & (df["block"] == "HONG-HARI")
            & (df["localbody"] == "SICHUSII"),
            # CHHATTISGARH
            (df["state"] == "CHHATTISGARH")
            & (df["district"] == "BEMETARA")
            & (df["block"] == "SAJA")
            & (df["localbody"] == "CHEEJGAON"),
            (df["state"] == "CHHATTISGARH")
            & (df["district"] == "BILASPUR")
            & (df["block"] == "MASTURI")
            & (df["localbody"] == "DEWREE"),
            (df["state"] == "CHHATTISGARH")
            & (df["district"] == "BILASPUR")
            & (df["block"] == "MASTURI")
            & (df["localbody"] == "DHURVAKARI"),
            # GOA
            (df["state"] == "GOA")
            & (df["district"] == "SOUTH GOA")
            & (df["block"] == "QUEPEM")
            & (df["localbody"] == "CAVOREM-PIRLA"),
            # HIMACHAL PRADESH
            (df["state"] == "HIMACHAL PRADESH")
            & (df["district"] == "MANDI")
            & (df["block"] == "MANDI SADAR")
            & (df["localbody"] == "BEER"),
            ...
            & (df["localbody"] == "BEER"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "BELAGAVI")
            & (df["block"] == "BYLAHONGAL")
            & (df["localbody"] == "MARADINAGALAPUR"),
            ...
            (df["state"] == "KARNATAKA")
            & (df["district"] == "CHAMARAJANAGARA")
            & (df["block"] == "HANURU")
            & (df["localbody"] == "SHAAGYA"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "CHITRADURGA")
            & (df["block"] == "CHALLAKERE")
            & (df["localbody"] == "THIMMANNANAYAKANAKOTE"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "CHITRADURGA")
            & (df["block"] == "HIRIYUR")
            & (df["localbody"] == "VANIVILASAPURA"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "DAKSHINA KANNADA")
            & (df["block"] == "BANTVAL")
            & (df["localbody"] == "BALEPUNI"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "DAKSHINA KANNADA")
            & (df["block"] == "BANTVAL")
            & (df["localbody"] == "KURNAD"),
            ...
            & (df["district"] == "DAKSHINA KANNADA")
            & (df["block"] == "MANGALURU")
            & (df["localbody"] == "ATHIKARIBETTU"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "HASSAN")
            & (df["block"] == "ARSIKERE")
            & (df["localbody"] == "J.C.PUR"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "MANDYA")
            & (df["block"] == "MANDYA")
            & (df["localbody"] == "MANDYA GRAMANATHARA"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "MANDYA")
            & (df["block"] == "PANDAVAPURA")
            & (df["localbody"] == "KURUBARABETTAHALLI"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "MANDYA")
            & (df["block"] == "PANDAVAPURA")
            & (df["localbody"] == "KURUBARABETTAHALLI"),
            ...
            & (df["district"] == "MANDYA")
            & (df["block"] == "PANDAVAPURA")
            & (df["localbody"] == "TIRUMALASAGARA"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "MYSURU")
            & (df["block"] == "HUNSUR")
            & (df["localbody"] == "UDBOORKAVAL"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "MYSURU")
            & (df["block"] == "SARAGURU")
            & (df["localbody"] == "KALLAMBALU"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "RAICHUR")
            & (df["block"] == "MANVI")
            & (df["localbody"] == "BAYAGWAT"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "RAMANAGARA")
            & (df["block"] == "RAMANAGARA")
            & (df["localbody"] == "HULIKERE GUNNUR"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "UTTARA KANNADA")
            & (df["block"] == "HONAVAR")
            & (df["localbody"] == "HALEMATH"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "UTTARA KANNADA")
            & (df["block"] == "HONAVAR")
            & (df["localbody"] == "MANKI GULADKERI"),
            (df["state"] == "KARNATAKA")
            & (df["district"] == "VIJAYAPURA")
            & (df["block"] == "VIJAYAPURA")
            & (df["localbody"] == "LOHGAON"),
        ]
    ...
    def data_name_concater():
        ...
        df["state_block_localbody"] = (
            df["state"] + df["block"] + df["localbody"]
        )
    ...
    def lgd_mapper():
        ...
        lgd.drop_duplicates("state_block_localbody", inplace=True)
        ...
        df1 = pd.merge(
            ...
            lgd,
            how="outer",
            left_on="state_block_localbody",
            right_on="state_block_localbody",
            validate="m:1",
            indicator=True,
            suffixes=["_data", "_LGD"],
        )
        ...
        not_lgd_mapped = df1[(df1["_merge"] == "left_only")][
            [
                ...
                "localbody_data",
                # "state_dist_block",
                "state_block_localbody",
            ]
        ]

        not_lgd_mapped = not_lgd_mapped.drop_duplicates(
            subset="state_block_localbody"
        )
        ...
        for state in state_list:
            ...
            if not lgd_iter_file.exists():
                if state not in (["MEGHALAYA", "MIZORAM", "NAGALAND"]):
                    ...
                    result = [
                        process.extractOne(i, lgd["state_block_localbody"])
                        for i in not_lgd_mapped["state_block_localbody"]
                    ]
                    ...
                    not_lgd_proxy_df = (
                        pd.DataFrame(
                            not_lgd_mapped["state_block_localbody"], index=None
                        )
                        .reset_index()
                        .drop("index", axis=1)
                    )
                    ...
                    df_proxy["state_block_localbody"] = df_proxy[
                        "state_block_localbody"
                    ].replace(mapper_dict)
                    ...
                    df1 = df1.rename(
                        columns={
                            ...
                            "block_LGD": "block",
                            "localbody_code": "localbody_lgd_code",
