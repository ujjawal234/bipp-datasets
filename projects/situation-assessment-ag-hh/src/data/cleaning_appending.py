...
# adding categories to yes or no questions
yes_no_label = {1: 1, 2: 0}

df["hh_member_has_bank_account"] = df["hh_member_has_bank_account"].map(yes_no_label)

df["hh_member_mgnreg_job_card"] = df["hh_member_mgnreg_job_card"].map(yes_no_label)

df["work_under_mgnrega"] = df["work_under_mgnrega"].map(yes_no_label)

df["hh_member_regd_farmers_org"] = df["hh_member_regd_farmers_org"].map(yes_no_label)

df["kisan_credit_card"] = df["kisan_credit_card"].map(yes_no_label)

df["soil_health_card"] = df["soil_health_card"].map(yes_no_label)

df["fert_applied_reco_of_shc"] = df["fert_applied_reco_of_shc"].map(yes_no_label)

df["ahc_nakul_swasthya_patra"] = df["ahc_nakul_swasthya_patra"].map(yes_no_label)


# adding categories of PM Fasal Bima Yojana
pmfby_label = {1: "Yes", 2: "No", 3: "Not applicable -3"}

df["pmfby_last365days"] = df["pmfby_last365days"].map(pmfby_label)
