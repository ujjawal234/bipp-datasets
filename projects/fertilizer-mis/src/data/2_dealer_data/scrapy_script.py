...
class DealerSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        list2 = (
            []
        )  # list2 stores dictionary which has stateid as key and statename as value
        ...
        for i in range(0, 6):
            ...
            if next_page is not None:
                ...
                yield scrapy.Request(
                    next_page, callback=self.collect_data, meta=meta
                )

    def collect_data(self, response):
        store_table = response.css(
            "#right > center > table:nth-child(2)"
        ).get()
        ...
        data = list_df[
            index
        ]  # couldn't run so don't know what should be in third bracket
        # print(data)

        file_path = (
            self.parent_folder
            + "/"
            + str(response.meta["state_name"])
            + "/"
            + str(response.meta["dealer_type"])
        )
