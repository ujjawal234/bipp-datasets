...
def read_folder(csv_folder):
    ...
    for d in df_list:
        ...
        )  # to remove trailing and leading spaces in each dataframe
    df_full = pd.concat(
        df_list, ignore_index=True
    )  # to concatenate dataframes in one dataframe
    return df_full


# function call
combined_dataframe = read_folder(
...
combined_dataframe.to_csv(
    r"bipp-datasets\projects\fertilizer-mis\data\interim\dealer_data.csv",
    index=False,
)
