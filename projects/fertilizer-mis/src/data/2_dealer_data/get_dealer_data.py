...
import os
options = webdriver.ChromeOptions()  #used in order to change download directory
options.add_experimental_option("prefs", {"download.default_directory": r"bipp-datasets\projects\fertilizer-mis\data\raw\2_dealer_data"})
...
driver.get("https://reports.dbtfert.nic.in/mfmsReports/getDealerInfoExcel")   #go to homepage
...
for x in range(1,no_of_states):   #iterates through the number of states
    ...
    drp_state=Select(driver.find_element_by_id("parameterStateName"))    # to select the states dropdown list 
    drp_state.select_by_index(x)    #selects each state by index
    driver.find_element_by_id("exporttoexcel").click()  #clicks on the download button


i=-2   #iterator used for old file names
drp_state=Select(driver.find_element_by_id("parameterStateName")) #to select the state dropdown list
...
for state in states_list:
    ...
    drp_state=Select(driver.find_element_by_id("parameterStateName"))   #to select the state dropdown list
    state_name = state.text   #state_name stores the visible text of the options
    new_file_name = state_name + '.csv'
    if(i==0):     #i=-1 is for the option select state, i=0 is for the first file
        old_file_name = "Report.csv"
    elif(i>=1):     
        old_file_name = "Report ({}).csv".format(i)
    if(i>=0):
        old_file = os.path.join(r"bipp-datasets\projects\fertilizer-mis\data\raw\2_dealer_data", old_file_name)
        new_file = os.path.join(r"bipp-datasets\projects\fertilizer-mis\data\raw\2_dealer_data", new_file_name)
