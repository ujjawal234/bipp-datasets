"""the data scraper script for nrlm f1b data."""
...
for row in all_names:
            ...
            try:
                ...
                for state, st_href in zip(state_names, state_hrefs):
                    ...
                    if state == row["state_name"]:
                        ...
                        state_table.insert(2,'state_name', state)
                    ...
                    if state == row["state_name"]:
                        ...
                        for district, di_href in zip(district_names, district_hrefs):
                            if district == row["district_name"]:
                                ...
                                district_table.insert(2,'state_name', state)
                                district_table.insert(3,'district_name', district)
                            ...
                            if district == row["district_name"]:
                                ...
                                for block, bl_href in zip(block_names, block_hrefs):
                                    if block == row["block_name"]:
                                        ...
                                        block_table.insert(2,'state_name', state)
                                        block_table.insert(3,'district_name', district)
                                        block_table.insert(4,'block_name', block)
                                    ...
                                    if block == row["block_name"]:
                                        ...
                                        for gp, gp_href in zip(gp_names, gp_hrefs):
                                            if gp == row["gp_name"]:
                                                ...
                                                gp_table.insert(2,'state_name', state)
                                                gp_table.insert(3,'district_name', district)
                                                gp_table.insert(4,'block_name', block)
                                                gp_table.insert(5,'gp_name', gp)
    ...
    if row["village_name"]!="NOT_FOUND":
        ...
        village_folder_path = Path.joinpath(gp_folder_path, row["village_name"].strip().replace(" ", "_"))
        village_name_corrected = re.sub(r"[^A-Za-z0-9_]", "", row["village_name"].strip().replace(" ", "_"))
                                                ...
                                                if gp == row["gp_name"]:
                                                    ...
                                                    for village, vi_href in zip(village_names, village_hrefs):
                                                        if village == row["village_name"]:
                                                            ...
                                                            village_table.insert(2,'state_name', state)
                                                            village_table.insert(3,'district_name', district)
                                                            village_table.insert(4,'block_name', block)
                                                            village_table.insert(5,'gp_name', gp)
                                                            village_table.insert(6,'village_name',village)
