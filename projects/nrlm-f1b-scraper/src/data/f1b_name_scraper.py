...
for state, st_href in zip(state_names, state_hrefs):
    ...
    while True:
        ...
        if next_page_class != "paginate_button next disabled":
            ...
            print("NEXT button exists for " + state + ". Scraping district names the next page...", end = "\n" )
            ...
            district_table = driver.find_elements(By.XPATH, "//*[@id='example']//a")
            district_names_next = [x.get_attribute("text") for x in district_table]
            district_names.extend(district_names_next)
            block_hrefs_next = [x.get_attribute("href") for x in district_table]
    ...
    for district, di_href in zip(district_names, district_hrefs):
        ...
        try: 
            ...
            if not district_json_path.exists():
                ...
                print("json for " + district + " doesn't exist. Proceeding for scraping...", end = "\n")
                ...
                block_table = driver.find_elements(By.XPATH, '//*[@id="example"]//a')
                ...
                while True:
                    ...
                    if next_page_class != "paginate_button next disabled":
                        ...
                        print("NEXT button exists for " + district + ". Scraping block names the next page...", end = "\n" )
                        ...
                        block_table = driver.find_elements(By.XPATH, "//*[@id='example']//a")
                        block_names_next = [x.get_attribute("text") for x in block_table]
                        block_names.extend(block_names_next)
                        block_hrefs_next = [x.get_attribute("href") for x in block_table]
                    ...
                    try:
                        ...
                        while True:
                            ...
                            if next_page_class != "paginate_button next disabled":
                                ...
                                print("NEXT button exists for " + block + ". Scraping grampanchayat names the next page...", end = "\n" )
                                ...
                                gp_hrefs_next = [x.get_attribute("href") for x in gp_table]
                        ...
                        for gp, gp_href in zip(gp_names, gp_hrefs):
                            ...
                            try:
                                ...
                                village_table = driver.find_elements(By.XPATH, '//*[@id="example"]//a')
                                village_names = [x.get_attribute("text") for x in village_table]
                                village_hrefs = [x.get_attribute("href") for x in village_table]
                                ...
                                while True:
                                    ...
                                    if next_page_class != "paginate_button next disabled":
                                        ...
                                        print("NEXT button exists for " + gp + ". Scraping village names the next page...", end = "\n" )
                                        ...
                                        village_table = driver.find_elements(By.XPATH, "//*[@id='example']//a")
                                        village_names_next = [x.get_attribute("text") for x in village_table]
                                        village_names.extend(village_names_next)
                                        village_hrefs_next = [x.get_attribute("href") for x in village_table]
                                ...
                                if len(village_names)==0:
                                    ...
                                    if (str.isascii(district)) and (str.isascii(block)) and (str.isascii(gp)):

                                        #creating village dictionary

                                        village_dict = {"state_name": state, "district_name": district,"block_name": block,"gp_name": gp, "village_name": "NOT_FOUND"}
                                        json_list.append(village_dict)

                                    else:

                                        print("Devanagari in name at {state}, {district}, {block}, {gp}, {village}. Hence, moving on to next village...", end="\n" )
                                    ...
                                    for village in village_names:
                                        ...
                                        if (str.isascii(district)) and (str.isascii(block)) and (str.isascii(gp)) and (str.isascii(village)):

                                            #creating village dictionary

                                            village_dict = {"state_name": state, "district_name": district,"block_name": block,"gp_name": gp, "village_name":village}
                                            json_list.append(village_dict)
                            ...
                            #sub-sub-except block begins
                                ...
                                print(f"No Such Element raised at {state} {district} {block} {gp}")
                                ...
                                no_of_pages_1 = Select(driver.find_element(By.XPATH, '//*[@id="example_length"]/label/select'))
                ...
                print(district_json_path, "exists and skipped. Moving to next district.")
                continue
